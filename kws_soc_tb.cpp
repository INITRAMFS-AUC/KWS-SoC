...
const char *help_str =
    ...
    "    --mic x.hex      : Path to I2S mic audio data hex file\n"
...
int wait_for_connection(int server_fd, uint16_t port, struct sockaddr *sock_addr, socklen_t *sock_addr_len) {
    ...
    // Non-blocking: lets the sim tick freely when the CPU is running
    // and no JTAG traffic is in flight (e.g. after GDB "continue").
...
int main(int argc, char **argv) {
    ...
    // lglen=24 means 16MB of memory, rddelay=1, ndummy=4
    FLASHSIM qspi_flash(24, xip_debug, 1, 4);
    ...
    for (int64_t cycle = 0; cycle < max_cycles || max_cycles == 0; ++cycle) {
        ...
        bool i2s_sck = top.p_i2s__sck__out.get<bool>();
        bool i2s_ws  = top.p_i2s__ws__out.get<bool>();

        bool i2s_sd = i2s_mic.step(i2s_sck, i2s_ws);
        top.p_i2s__sd.set<bool>(i2s_sd);

        // I2S diagnostics: print first SCK edge, every WS transition, and periodic cycle count
        if (i2s_debug) {
        ...
        // Heartbeat: every 10M cycles print sim status so user knows it's alive
        if (cycle > 0 && cycle % 10000000 == 0) {
            bool uart_tx_now = top.p_uart__tx.get<bool>();
            printf("[Sim] Cycle " I64_FMT "  uart_tx=%d\n", cycle, (int)uart_tx_now);
            fflush(stdout);
        }
                    ...
                    if (replay_jtag) {
                        ...
                        if (rx_remaining < 0) {
                            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                                // No JTAG bytes ready — tick the sim one cycle
                                // so the CPU, I2S, timers, etc. keep running.
                                rx_remaining = 0;
                                step = true;
                            } else {

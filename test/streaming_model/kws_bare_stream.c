...
void __attribute__((interrupt("machine"))) kws_trap_handler(void)
{
    ...
    /* dmac_irq: DMA burst complete — clear ICR latch */
    DMAC->icr = 1u;

    /* DMA has populated dma_batch — extract int8 Q7 samples */
...
int main(void)
{
    ...
    while (1) {
        ...
        /* Snapshot hop buffer; re-arm DMA so audio capture continues */

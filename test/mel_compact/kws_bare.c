...
#ifndef I2S_CLK_DIV
/* cfg_div=35 → SCK half-period=36 sys-clk cycles → SCK=500kHz → 7812 Hz sample rate.
 * At 36 MHz: sample_rate = 36e6 / (2*(cfg_div+1)*64) where 64 = 32 L-bits + 32 R-bits.
 * I2S peripheral stores both L and R in the FIFO; firmware reads L and discards R.
 * 70 was wrong: it gives 3953 Hz (quarter rate). */
...
static inline void csr_enable_cycle_counter(void) {
    /* Hazard3 inhibits mcycle/minstret by default (mcountinhibit[0]=1).
     * Clear the inhibit register so mcycle increments every active clock cycle. */
    asm volatile ("csrw 0x320, zero");  /* 0x320 = mcountinhibit */
}
...
int main(void) {
    ...
    /* Debug: poll FIFO directly to check if I2S data arrives at all */
    {
        uart_puts("POLL:");
        volatile uint32_t *fifo = (volatile uint32_t *)0x40008008UL;
        /* spin until we read 8 non-zero values, or 10M cycles */
        int got = 0;
        for (uint32_t t = 0; t < 10000000u && got < 8; t++) {
            uint32_t v = *fifo;
            if (v) { uart_puthex(v); uart_putc(' '); got++; }
        }
        uart_puts(got ? "OK\r\n" : "TIMEOUT\r\n");
    }

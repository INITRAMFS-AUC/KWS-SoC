...
```bash
  ...
  # kws_bare_main.c hard-codes WIDTH=8 + DS_EN=1 itself, so no I2S_WIDTH_MODE here.
  make test \
      I2S_CLK_DIV=5            # 36 MHz / (128*6) = 46.875 kHz raw → 15.6 kHz ring → 7.8 kHz model

  # Build Verilator simulator with matching CLK_MHZ / UART_BAUD_RATE.
...
```bash
  make test \
      ...
      I2S_WIDTH_MODE=I2S_CONF_WIDTH_8     # match int8 path
